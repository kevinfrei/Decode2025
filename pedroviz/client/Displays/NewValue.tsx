...
export function NewValue(): ReactElement {
  ...
  const label = (
    <Select onChange={(_, data) => setIsVal(data.value === 'Value')}>
      <option selected={isVal}>Value</option>
      <option selected={!isVal}>Variable</option>
    </Select>
  );
  ...
  const valOrVar = isVal ? (
    ...
    <Select onChange={(_, data) => setVarStr(data.value)}>
      {[
        ...allNames.keys().map((nv) => (
          <option key={nv} selected={nv === varStr}>
            {nv}
          </option>
        )),
      ]}
